...
fetch('https://parlaynplay.herokuapp.com/get-straight-bets-history', {
    mode: 'cors',
    method: 'POST',
    credentials: 'include',
...
.then(data => {
    if(data.success) {
        ...
        }) : straight_tbody.innerHTML = 'You have no Straight Bets'
    }else{
...
fetch('https://parlaynplay.herokuapp.com/get-parley-bets-history', {
    mode: 'cors',
    method: 'POST',
    credentials: 'include',
    headers: {'Content-Type': 'application/json', 'Accept': 'application/json', 'Origin':'http://localhost:5501;'},
...
.then(data => {
    if(data.success) {
        ...
        }) : parley_tbody.innerHTML = 'You have no Parlay Bets'
    }else{
        console.log(data.error);
        toast('An error occured from the server');
    }
...
function renderStraightBets(doc) {
    ...
    result.innerHTML = doc.won ? `<div class="green-dot"></div>` : `<div class="red-dot"></div>`;
...
function renderParleyBets(doc) {
    ...
    viewTicketBtn.onclick = () => {
        ...
        const win = create('h5');
        win.innerHTML = `Potential Win: <span class="${doc.won ? 'green-text' : 'red-text'} bebas">$${doc['amount to win']}</span> ${doc.won ? '<span class="green-text bebas">| Won</span>' : '<span class="red-text bebas">| Lost</span>'}`
